                    ...
                    <table class="table table-striped">
                        ...
                        @foreach ($content as $index => $item)
                            <tr>
                                ...
                                <td>
                                    <form class="d-inline m-0 p-0"
                                          action="{{ route( 'public.scard.increment', ['rawId' => $index]) }}"
                                          method="post">
                                        @csrf
                                        <button type="submit" class="btn btn-link m-0 p-0 small" role="link"><i
                                                class="fas fa-plus"></i></button>
                                    </form>
                                </td>
                                <td>
                                    <form class="d-inline m-0 p-0"
                                          action="{{ route( 'public.scard.decrement', ['rawId' => $index]) }}"
                                          method="post">
                                        @csrf
                                        <button type="submit" class="btn btn-link m-0 p-0 small" role="link"><i
                                ...
                                <td>
                                    <form class="d-inline m-0 p-0"
                                          action="{{ route( 'public.scard.delete', ['rawId' => $index]) }}"
                                          method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit" class="btn btn-link m-0 p-0 small" role="link"><i
