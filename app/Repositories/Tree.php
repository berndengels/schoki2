...
class Tree
{
	...
	public function make($parent, $position = 0, $data = array()) {
		...
		// update left indexes
		$ref_lft = false;
		if(!$parent['children'] || !isset($parent['children'][$position])) {
		...
		$strCols = implode(",", $this->options['structure']);
		$strVals = '?'.str_repeat(',?', count($this->options['structure']) - 1);
		$sql[] = "INSERT INTO ".$this->options['structure_table']." (".$strCols.") VALUES (".$strVals.")";
		$tmp = array();
	...
	public function analyze($get_errors = false) {
		...
		if((int)$this->db->query("SELECT ".$this->options['structure']['left']." AS res FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']["parent_id"]." = 0")->fetchColumn() !== 1) {
		...
		if((int)$this->db->query("
			...
			WHERE
				...
				(SELECT COUNT(".$this->options['structure']['id'].") FROM ".$this->options['structure_table']." WHERE ".$this->options['structure']['id']." = s.".$this->options['structure']["parent_id"].") = 0")->fetchColumn() > 0
		) {
			$report[] = "Missing parents.";
		}
		if(
			(int)$this->db->query("SELECT MAX(".$this->options['structure']['right'].") AS res FROM ".$this->options['structure_table'])->fetchColumn() / 2 !=
			(int)$this->db->query("SELECT COUNT(".$this->options['structure']['id'].") AS res FROM ".$this->options['structure_table'])->fetchColumn()
		) {
			$report[] = "Right index does not match node count.";
		}
		if(
			(int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']['right'].") AS res FROM ".$this->options['structure_table'])->fetchColumn() !=
			(int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']['left'].") AS res FROM ".$this->options['structure_table'])->fetchColumn()
		) {
			$report[] = "Duplicates in nested set.";
		}
		if(
			(int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']['id'].") AS res FROM ".$this->options['structure_table'])->fetchColumn() !=
			(int)$this->db->query("SELECT COUNT(DISTINCT ".$this->options['structure']['left'].") AS res FROM ".$this->options['structure_table'])->fetchColumn()
		) {
	...
	public function reconstruct($analyze = true) {
		...
		$counter = 2;
		if(!$this->db->query("SELECT COUNT(*) FROM temp_tree")->fetchColumn()) {
			return false;
		}
		$this->db->nextr();
		...
		if(!$this->db->exec("DELETE FROM temp_tree WHERE ".$this->options['structure']["parent_id"]." = 0")) {
			return false;
		}
		...
		while ($counter <= $maxcounter) {
			...
			if($this->db->nextr()) {
				...
				if(!$this->db->exec("DELETE FROM temp_tree WHERE ".$this->options['structure']['id']." = ".$tmp)) {
					return false;
				}
				$counter++;
				$currenttop++;
		...
		if(!$this->db->exec("TRUNCATE TABLE ".$this->options['structure_table']."")) {
			return false;
		}
		if(!$this->db->exec("" .
		...
		foreach($nodes as $node) {
			...
			$this->db->exec("UPDATE ".$this->options['structure_table']." SET ".$this->options['structure']['position']." = ".$last_position." WHERE ".$this->options['structure']['id']." = ".(int)$node[$this->options['structure']['id']]);
			$last_position++;
		}
	...
	public function dump() {
		...
		foreach($nodes as $node) {
			...
			echo $node[$this->options['structure']['id']]." ".$node['name']." (".$node[$this->options['structure']['left']].",".$node[$this->options['structure']['right']].",".$node[$this->options['structure']["level"]].",".$node[$this->options['structure']["parent_id"]].",".$node[$this->options['structure']["position"]].")" . "\n";
		}
		echo str_repeat("-",40);
		echo "\n\n";
	}
